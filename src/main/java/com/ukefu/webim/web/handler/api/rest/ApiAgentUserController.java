...
public class ApiAgentUserController extends Handler{
    ...
    public ResponseEntity<RestResult> list(HttpServletRequest request , @Valid String q) {
        return new ResponseEntity<>(new RestResult(RestResultType.OK , agentUserRepository.findByAgentnoAndOrgi(super.getUser(request).getId() , super.getOrgi(request))), HttpStatus.OK);
    }
}